...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)
static void mtktv_cpufreq_ready_work_cb(struct work_struct *work);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)
struct delayed_work mtktv_cpufreq_ready_work;
#endif
...
static void mtktv_boost_enable(unsigned int bEnable, unsigned int rate)
{
	...
	policy->min_freq_req, policy->max_freq_req);
#else
	CPUFREQ_DEBUG("[mtktv-cpufreq] min = %d, max = %d\n",
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)
static const struct proc_ops mtktv_voltage_fileops = {
	.proc_open       = mtktv_voltage_op_open,
	.proc_write      = mtktv_voltage_op_write,
	.proc_read       = seq_read,
	.proc_lseek     = seq_lseek,
	.proc_release    = mtktv_voltage_op_release,
};

static const struct proc_ops mtktv_voltage_core_fileops = {
	.proc_open       = mtktv_voltage_core_op_open,
	.proc_write      = mtktv_voltage_core_op_write,
	.proc_read       = seq_read,
	.proc_lseek     = seq_lseek,
	.proc_release    = mtktv_voltage_core_op_release,
};

static const struct proc_ops mtktv_frequency_fileops = {
	.proc_open       = mtktv_frequency_op_open,
	.proc_write      = mtktv_frequency_op_write,
	.proc_read       = seq_read,
	.proc_lseek     = seq_lseek,
	.proc_release    = mtktv_frequency_op_release,
};

static const struct proc_ops mtktv_temperature_fileops = {
	.proc_open       = mtktv_temperature_op_open,
	.proc_write      = mtktv_temperature_op_write,
	.proc_read       = seq_read,
	.proc_lseek     = seq_lseek,
	.proc_release    = mtktv_temperature_op_release,
};

static const struct proc_ops mtktv_temperature_info_fileops = {
	.proc_open       = mtktv_temperature_info_op_open,
	.proc_read       = seq_read,
	.proc_lseek     = seq_lseek,
	.proc_release    = mtktv_temperature_info_op_release,
};

static const struct proc_ops mtktv_boost_list_fileops = {
	.proc_open      = mtktv_boost_list_op_open,
	.proc_write     = mtktv_boost_list_op_write,
	.proc_read      = seq_read,
	.proc_lseek    = seq_lseek,
	.proc_release   = mtktv_boost_list_op_release,
};
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)
	schedule_delayed_work(&mtktv_cpufreq_ready_work, HZ);
#endif
...
#endif
{
	...
	policy = cpufreq_cpu_get_raw(cpu);
	if (!policy) {
		pr_emerg("cpu policy is not initialized.\n");
		schedule_delayed_work(&mtktv_cpufreq_ready_work, HZ);
		return;
	}
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)
	cancel_delayed_work(&mtktv_cpufreq_ready_work);
#endif
}
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 15, 0)
//use work repleace in the probe
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)
	INIT_DELAYED_WORK(&mtktv_cpufreq_ready_work, mtktv_cpufreq_ready_work_cb);
#endif
