...
fusionees_proc_show(struct seq_file *m, void *v)
{
     ...
     direct_list_foreach(fusionee, dev->fusionee.list) {
          seq_printf(m,
                  "(%5d) 0x%08llx (%4d packets waiting, %7ld received, %7ld sent) - '%s'\n",
                  fusionee->pid, fusionee->id,
                  fusionee->packets.count, atomic_long_read(&fusionee->rcv_total),
                  atomic_long_read(&fusionee->snd_total),
                  fusionee->exe_file);
...
static const struct proc_ops fusionees_proc_fops = {
     .proc_open    = fusionees_proc_open,
     .proc_read    = seq_read,
...
#if defined(CONFIG_TREE_RCU) || defined(CONFIG_TREE_PREEMPT_RCU) || defined(CONFIG_TINY_RCU) || defined(rcu_read_lock) || defined(CONFIG_PREEMPT_RCU)
                    rcu_read_lock();
#else
...
#if defined(CONFIG_TREE_RCU) || defined(CONFIG_TREE_PREEMPT_RCU) || defined(CONFIG_TINY_RCU) || defined(rcu_read_unlock) || defined(CONFIG_PREEMPT_RCU)
                    rcu_read_unlock();
#else
