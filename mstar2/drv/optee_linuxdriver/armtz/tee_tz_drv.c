...
#define RAMLOG_PATH "/data/tee/ramlog_enable.bin"
#define RAMLOG_ENABLE_KEY_SIZE 16
...
static struct tee_shm *handle_rpc_alloc(struct tee_tz *ptee, size_t size)
{
	...
	memset(&rpc_alloc, 0 , sizeof(struct tee_rpc_alloc));
			...
			uintptr_t parg32, struct teesmc32_arg *arg32)
{
	...
	while (true) {
		...
		//Linaro orignal behavior
		TIME_PRINTK_START("SMC ENTER",start, 0);
		tee_smc_call(&param);
		TIME_PRINTK_END("SMC EXIT",start,end, 0);
		ret = param.a0;
		...
		if (ret == TEESMC_RETURN_ETHREAD_LIMIT) {
			...
			/* Process the RPC. */
			e_unlock_teez(ptee);
			TIME_PRINTK_START("SMC RPC ENTER",start, TEESMC_RETURN_GET_RPC_FUNC(param.a0));
			funcid = handle_rpc(ptee, &param);
			TIME_PRINTK_END("SMC RPC EXIT",start,end, 0);
			e_lock_teez(ptee);
		} else {
			break;
		}
...
#ifdef CONFIG_MSTAR_CHIP
static int tz_set_log_level(uint32_t level, struct tee *tee)
{
	struct smc_param param = { 0 };

	BUG_ON(!tee || !tee->priv);

	printk("\033[0;32;31m [SECARM] %s %d Set Log Level %d\033[m\n",__func__,__LINE__,level);
	param.a0 = 0xb2005858;
	param.a1 = level;
	tee_smc_call(&param);
...
static int tz_configure_crc_args(int miu,uint32_t offset,uint32_t size)
{
	struct smc_param param = { 0 };
...
static int tz_pm51ctl(uint32_t cmd, struct tee *tee)
{
	struct smc_param param = { 0 };
#ifdef CONFIG_MSTAR_CHIP
	struct tee_tz *ptee = NULL;

	BUG_ON(!tee || !tee->priv);
	ptee = tee->priv;
#endif

	printk("\033[0;32;31m %s %d PM51CTL %d\033[m\n",__func__,__LINE__,cmd);
	param.a0 = 0xb200685C;
	param.a1 = cmd;
...
static int register_outercache_mutex(struct tee_tz *ptee, bool reg)
{
	...
	uintptr_t paddr = 0;
#ifdef CONFIG_MSTAR_CHIP
	struct semaphore current_event = (struct semaphore) __SEMAPHORE_INITIALIZER(current_event, 0);
	u32 slot = 0xffffffff;
	u32 retry_count = 0;
#endif
...
static int configure_ramlog(struct tee_tz *ptee)
{
	struct smc_param param = { 0 };
	void *ramlog_vaddr;
	int ret = 0;
	int dataSize = RAMLOG_ENABLE_KEY_SIZE;
	...
	pRamlogEnableFile = filp_open(RAMLOG_PATH ,O_RDONLY ,0);
       if (!IS_ERR(pRamlogEnableFile)) {
		printk(KERN_INFO "%s opened successfully!\n", RAMLOG_PATH);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)
		read_ret = kernel_read(pRamlogEnableFile, vaddr, dataSize, &pRamlogEnableFile->f_pos);
#else
		old_fs = get_fs();
		set_fs(KERNEL_DS);
...
fail:
	...
	mutex_lock(&ptee->mutex);
	param.a0 = TEESMC32_ST_FASTCALL_RAMLOG;
	tee_smc_call(&param);
	mutex_unlock(&ptee->mutex);

	if(paddr != 0)
		tee_shm_pool_free(DEV, ptee->shm_pool, paddr, &freeSize);

	if (param.a0 != TEESMC_RETURN_OK) {
		dev_err(DEV, "ramlog not available: %X", (uint)param.a0);
		ret = 0;
		goto out;
	}
	...
	ret = tee_ramlog_init_addr((unsigned long)ramlog_vaddr,param.a2);
	if (ret) {
		dev_err(DEV,"ram log have been taken \n");
		goto out;
	}
	tee_ramlog_tsk= kthread_run(tee_ramlog_loop, NULL, "tee_ramlog_loop");
out:
...
#ifdef CONFIG_MSTAR_CHIP
//static struct task_struct *tee_mbx_handle; //for debug poll mode
#endif
...
#define NONE                    "\033[m"
#ifdef CONFIG_MSTAR_CHIP
static int tz_tee_resume(struct platform_device *dev)
{
	struct smc_param param = { 0 };
