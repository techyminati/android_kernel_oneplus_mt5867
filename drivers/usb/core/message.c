		...
		do {
			if (!wait_for_completion_timeout(&ctx.done, timeout_step)) {
				...
				if(total_time >= expire)
				{
					...
					dev_err(&urb->dev->dev,
						"[USB]%s timed out %d/%d ms on ep%d%s len=%u/%u\n",
						current->comm,
...
#endif /* CONFIG_MP_USB_STR_PATCH */
				{
					...
					dev_err(&urb->dev->dev,
#if defined(CONFIG_SUSPEND) && defined(CONFIG_MP_USB_STR_PATCH)
						"[USB][%d]%s time %d/%d ms, CSC happen on ep%d%s len=%u/%u\n",
						is_suspending(),
#else
						"[USB]%s time %d/%d ms, CSC happen on ep%d%s len=%u/%u\n",
#endif
						current->comm,
						jiffies_to_msecs(total_time),
						timeout,
	...
	while(1)
	{
		...
		if(timeleft == 0)
		{
			if(is_suspending())
			{
				if (printk_ratelimit()) {
					dev_err(&io->dev->dev,
						"%s, cancel io on suspend, %d/%d timeout_step:%d sec\n",
						__func__, io->count, io->entries, timeout_step);
				}
				usb_sg_cancel(io);
				//break;
			}
