	...
	if ((to_usb_device(ddev)->speed == USB_SPEED_HIGH) &&
		...
		if (to_usb_device(ddev)->parent) {
			if (to_usb_device(ddev)->parent->parent == NULL) {
				/* set EOF1 to 24 us for ISO later */
...
#if (MP_USB_MSTAR==1)
	/* Ttimeing compability issue, delay 500ms before
	 * get Config Desc for LG AN-VC400 Camera.
	 */
	if((le16_to_cpu(dev->descriptor.idVendor) == 0x043e) &&
			(le16_to_cpu(dev->descriptor.idProduct) == 0x3009))
	{
		//printk("[USB] delay 500ms for LG AN-VC400 Camera\n");
		mdelay(500);
	}
#endif 
