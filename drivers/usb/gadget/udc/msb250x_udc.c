...
#define MIU_SEL_REG  (0x19*2-1)
void MIU_select_setting_udc(uintptr_t USBC_base)
{
	int miu_sel_reg;

	miu_sel_reg = readb((void*)(USBC_base + MIU_SEL_REG));
...
#ifdef IRQ_AFFINITY_UDC
		u32 cpu_num;

		/*	get irq affinity setting */
		cpu_num = num_online_cpus();
		if (cpu_num > IRQ_AFFINITY_UDC) {
			dev_crit(&udc->dev, "Set irq affinity udc %d\n", IRQ_AFFINITY_UDC);
			irq_set_affinity_hint(irq, cpumask_of(IRQ_AFFINITY_UDC));
		}
#endif
