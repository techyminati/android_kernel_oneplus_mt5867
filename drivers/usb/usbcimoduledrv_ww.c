...
#define USB_CIMODULE_MEDIA_MAX_PAGE_ORDER      	9
#define USB_CIMODULE_MEDIA_MAX_KMEM_SIZE    	0x200000		// 0x80000 = 1024*4*2^(USB_CIMODULE_MEDIA_MAX_PAGE_ORDER-1))
...
#define MATCHED_INTF_CLASS			0xEF
#define MATCHED_INTF_SUBCLASS		0x07

#define IAD_FUNCTION_CLASS			0xEF
#define IAD_FUNCTION_SUBCLASS		0x07
#define IAD_FUNCTION_PROTOCOL		0x01

#define COMMAND_INTF_CLASS			0xEF
#define COMMAND_INTF_SUBCLASS		0x07
#define COMMAND_INTF_POTOCOL		0x01

#define MEDIA_INTF_CLASS			0xEF
#define MEDIA_INTF_SUBCLASS			0x07
...
struct usb_cimodule_info {
	...
	unsigned char m_arReserve[RES_NUM];
};

static int usb_ci_debug;
module_param(usb_ci_debug, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(usb_ci_debug, "Print USB Debug message in USB CI CAM");
...
static bool alloc_write_buf = TRUE;
static bool alloc_read_buf = TRUE;
static bool alloc_write_command_buf = TRUE;
static unsigned char *m_pbMediaInBuf_init = NULL;
...
static unsigned long cimodule_get_driver_version(struct usb_cimodule *i_ptDevData, unsigned long i_dwArg)
{
	...
	nRetval = copy_to_user((unsigned int *)i_dwArg, &dwDriverVersion, sizeof(int));
	if (nRetval) {
...
static unsigned long cimodule_cancel_media_transfer(struct file *i_pFile, struct usb_cimodule *i_ptDevData)
{
	...
	if ((i_pFile->f_flags & FLAG_MASK) == MEDIA_INTF_READ_ACCESS_MODE) {
	...
	} else if ((i_pFile->f_flags & FLAG_MASK) == MEDIA_INTF_WRITE_ACCESS_MODE) {
...
static long cimodule_ioctl(struct file *i_pFile, unsigned int i_dwOp, unsigned long i_dwArg)
{
	...
	if (!ptDevData->m_pbConnectFlag) {
		LOG_ERR("CI+ CAM has disconnected");
		return -ENODEV;
	}
...
static void cimodule_delete(struct kref *i_tKref)
{
	...
	if (ptDevData->m_pbCmdInBuf) {
		kfree(ptDevData->m_pbCmdInBuf);
		ptDevData->m_pbCmdInBuf = NULL;
	}
	/*
	if (ptDevData->m_pbMediaInBuf) {
		kfree(ptDevData->m_pbMediaInBuf);
		ptDevData->m_pbMediaInBuf = NULL;
	}
	*/
...
static int command_intf_open(struct inode *i_tInode, struct file *o_pFile)
{
	...
	LOG_INFO("command interface open succeessfully %p\n", ptDevData);
	return 0;
}
/*
static int command_intf_mmap(struct file *i_pFile, struct vm_area_struct *i_ptVma)
...
static ssize_t command_intf_write(struct file *i_pFile, const char *i_pbBuffer, size_t i_dwCount, loff_t *i_pPpos)
{
	...
	if (usb_ci_debug >= 1) {
		LOG_ERR("enter:i_dwCount=0x%x", i_dwCount);
	}
    ...
    if (alloc_write_command_buf) {
		alloc_write_command_buf = false;
		LOG_CRIT("usb dma memory alloc command write buffer only one time.\n");
		ptDevData->m_pbCmdOutBuf = usb_alloc_coherent(ptDevData->m_ptUsbDev, USB_CIMODULE_COMMAND_MAX_KMEM_SIZE, GFP_KERNEL,
						 &ptDevData->m_pbCmdOutUrb->transfer_dma);

		if (!ptDevData->m_pbCmdOutBuf) {
			LOG_CRIT("usb dma memory alloc failed for command interface");
			mutex_unlock(&ptDevData->m_tCmdOutMutex);
			return -ENOMEM;
		}
    }
	...
	if (copy_from_user(ptDevData->m_pbCmdOutBuf, i_pbBuffer, i_dwCount)) {
		mutex_unlock(&ptDevData->m_tCmdOutMutex);
		return -EFAULT;
	}
	...
	//enable USB DMA mode
	ptDevData->m_pbCmdOutUrb->transfer_flags |= URB_NO_TRANSFER_DMA_MAP;
    ...
    /*
	if (ptDevData->m_pbCmdOutBuf) {
		usb_free_coherent(ptDevData->m_ptUsbDev, USB_CIMODULE_COMMAND_MAX_KMEM_SIZE, ptDevData->m_pbCmdOutBuf, ptDevData->m_pbCmdOutUrb->transfer_dma);
		ptDevData->m_pbCmdOutBuf = NULL;
	}
	*/
	...
	if (usb_ci_debug >= 1) {
		LOG_ERR("end:u32WriteSize=0x%x", u32WriteSize);
	}
	return u32WriteSize;
}
...
static void command_intf_read_complete(struct urb *i_ptUrb)
{
	int nStatus;
	struct usb_cimodule *ptDevData = NULL;
	//LOG_ERR("command_intf_read_complete len = %d",i_ptUrb->actual_length);
...
static ssize_t command_intf_read(struct file *i_pFile, char *i_pbBuffer, size_t i_dwCount, loff_t *i_pPpos)
{
	...
	if (ptDevData->m_nCmdInErr < 0)	{
		...
		LOG_ERR("command read error, return = %d", nRetval);
		return nRetval;
	}
	...
	nRetval = copy_to_user(i_pbBuffer, ptDevData->m_pbCmdInBuf, dwReadSize);
	if (nRetval) {
		LOG_ERR("copy data to user error");
		return nRetval;
	}
	if (usb_ci_debug >= 1) {
		LOG_ERR("end:dwReadSize=0x%x", dwReadSize);
	}
	return dwReadSize;
...
static int command_intf_release(struct inode *i_ptInode, struct file *i_pFile)
{
	...
	LOG_INFO("command interface release ptDevData %p\n", ptDevData);
	if (!ptDevData)	{
		LOG_ERR("private data is null");
		return -ENODEV;
	}
...
static int media_intf_open(struct inode *i_ptInode, struct file *i_pFile)
{
	...
	LOG_INFO("enter end ");
	LOG_INFO("media interface open succeessfully %p\n", ptDevData);
...
static ssize_t media_intf_write(struct file *i_pFile, const char *i_pbBuffer, size_t i_dwCount, loff_t *i_pPpos)
{
	...
	if ((i_pFile->f_flags & FLAG_MASK) != MEDIA_INTF_WRITE_ACCESS_MODE) {
    ...
    if(alloc_write_buf) {
	    alloc_write_buf = FALSE;
	    LOG_ERR("only first time alloc media write buffer");
	    ptDevData->m_pbMediaOutBuf = usb_alloc_coherent(ptDevData->m_ptUsbDev, USB_CIMODULE_MEDIA_MAX_KMEM_SIZE, GFP_KERNEL,
					 &ptDevData->m_pbMediaOutUrb->transfer_dma);
		if (!ptDevData->m_pbMediaOutBuf)	{
			LOG_ERR("usb dma memory alloc failed for media interface");
			mutex_unlock(&ptDevData->m_tMediaOutMutex);
			return -ENOMEM;
		}
    }
	...
	if (!access_ok((void __user *)i_pbBuffer,  i_dwCount))
	{
		memcpy(ptDevData->m_pbMediaOutBuf, (void __user *)i_pbBuffer, i_dwCount);
	}
	else {
		if (copy_from_user(ptDevData->m_pbMediaOutBuf, i_pbBuffer, i_dwCount)) {
			mutex_unlock(&ptDevData->m_tMediaOutMutex);
			LOG_ERR("address error1 and user addr is 0x%lx and kernel addr is 0x%lx",i_pbBuffer,ptDevData->m_pbMediaOutBuf);
			return -EFAULT;
		}
	}
	//bulk transfer
	if (usb_ci_debug >= DBG_LVL) {
		for (i = 0 ; i < PRINT_NUM; i++) {
		   LOG_ERR("write data = 0x%x", ptDevData->m_pbMediaOutBuf[i]);
		}
	}
	...
	ptDevData->m_pbMediaOutUrb->transfer_flags |= URB_NO_TRANSFER_DMA_MAP;
	...
	nRetval = wait_event_interruptible_timeout(ptDevData->m_tMediaOutWait, ptDevData->m_dwMediaOutCondition, msecs_to_jiffies(USB_ISR_TIMEOUT));
	if (nRetval <= 0) {

	    usb_kill_urb(ptDevData->m_pbMediaOutUrb);
	    LOG_ERR("media write error kill the urb");
	    return -ETIMEDOUT;
	}
#if 0
	if (nRetval > 0) {
		...
		if (ptDevData->m_pbMediaOutBuf) {
			usb_free_coherent(ptDevData->m_ptUsbDev, USB_CIMODULE_MEDIA_MAX_KMEM_SIZE, ptDevData->m_pbMediaOutBuf, ptDevData->m_pbMediaOutUrb->transfer_dma);
			ptDevData->m_pbMediaOutBuf = NULL;
		}

	}
	if (nRetval == 0) {
		LOG_ERR("media write timeout");
		if (ptDevData->m_pbMediaOutBuf) {
			usb_kill_urb(ptDevData->m_pbMediaOutUrb);
			usb_free_coherent(ptDevData->m_ptUsbDev, USB_CIMODULE_MEDIA_MAX_KMEM_SIZE, ptDevData->m_pbMediaOutBuf, ptDevData->m_pbMediaOutUrb->transfer_dma);
			ptDevData->m_pbMediaOutBuf = NULL;
    ...
    //LOG_INFO("write done (0x%x)\n",u32WriteSize);
	if (usb_ci_debug >= 1) {
		LOG_ERR("end:u32WriteSize=0x%x , nRetval%d ", u32WriteSize, nRetval);
	}
	return u32WriteSize;
}
...
static ssize_t media_intf_read(struct file *i_pFile, char *i_pbBuffer, size_t i_dwCount, loff_t *i_pPpos)
{
	...
	ptDevData->m_pbMediaInUrb = usb_alloc_urb(0, GFP_KERNEL);
	if (!ptDevData->m_pbMediaInUrb)	{
	...
	if (alloc_read_buf)
	{
		ptDevData->m_pbMediaInBuf = kmalloc(USB_CIMODULE_MEDIA_MAX_KMEM_SIZE, GFP_KERNEL);
        if (!ptDevData->m_pbMediaInBuf)
        {
			LOG_CRIT("media read buffer alloc failed");
			mutex_unlock(&ptDevData->m_tMediaInMutex);
			return -ENOMEM;
		}
		alloc_read_buf = FALSE;
	    LOG_ERR("only first time alloc media read buffer ");
	}
	if (!ptDevData->m_pbMediaInBuf)
	{
		LOG_CRIT("media read buffer is NULL");
		mutex_unlock(&ptDevData->m_tMediaInMutex);
		return -ENOMEM;
	}
	*/
	if (!m_pbMediaInBuf_init)
	{
		LOG_CRIT("media read buffer is NULL");
		mutex_unlock(&ptDevData->m_tMediaInMutex);
		return -ENOMEM;
	...
	usb_fill_bulk_urb(ptDevData->m_pbMediaInUrb, ptDevData->m_ptUsbDev, nPipe, /*i_pbBuffer*//*ptDevData->m_pbMediaInBuf*/m_pbMediaInBuf_init, i_dwCount, media_intf_read_complete, ptDevData);
	...
	nRetval = wait_event_interruptible_timeout(ptDevData->m_tMediaInWait, ptDevData->m_dwMediaInCondition, msecs_to_jiffies(USB_ISR_TIMEOUT));
	if (nRetval == 0) {
		LOG_ERR("media read timeout,kill and free urb");
		usb_kill_urb(ptDevData->m_pbMediaInUrb);
		usb_free_urb(ptDevData->m_pbMediaInUrb);
		return -ETIMEDOUT;
	} else if (nRetval < 0) {
		LOG_ERR("media read error");
		usb_kill_urb(ptDevData->m_pbMediaInUrb);
		usb_free_urb(ptDevData->m_pbMediaInUrb);
		return -ERESTARTSYS;
	}
	...
	if (!access_ok((void __user *)i_pbBuffer,  dwReadSize))
	{
		memcpy((void __user *)i_pbBuffer,m_pbMediaInBuf_init,dwReadSize);
	}
	else {
		nRetval = copy_to_user(i_pbBuffer, /*ptDevData->m_pbMediaInBuf*/m_pbMediaInBuf_init, dwReadSize);
		if (nRetval) {
			LOG_ERR("copy media read data to user error");
			return nRetval;
		}
	}
	...
	if (usb_ci_debug >= DBG_LVL) {
		for (i = 0; i < PRINT_NUM; i++) {
		   LOG_ERR("read data = 0x%x", i_pbBuffer[i]);
		}
	}
	if (usb_ci_debug >= 1) {
		LOG_ERR("end:dwReadSize=0x%x", dwReadSize);
...
static int media_intf_release(struct inode *i_ptInode, struct file *i_pFile)
{
	...
	LOG_INFO("media_intf_release ptDevData %p\n", ptDevData);
	...
	if ((i_pFile->f_flags & FLAG_MASK) == MEDIA_INTF_READ_ACCESS_MODE) {
		if (!(&ptDevData->m_pbMediaInUrb->use_count)) {
			LOG_INFO("ptDevData->m_pbMediaInUrb->use_count is null.\n");
		}
		if (!ptDevData->m_pbMediaInUrb) {
			LOG_INFO("ptDevData->m_pbMediaInUrb is null.\n");
		}
		if (ptDevData->m_pbMediaInUrb) {
			if (atomic_read(&ptDevData->m_pbMediaInUrb->use_count)) {
				LOG_INFO("do not unlink the media read urb");
	    ...
	    if (ptDevData->m_pbMediaOutUrb) {
			if (atomic_read(&ptDevData->m_pbMediaOutUrb->use_count)) {
				LOG_INFO("do not unlink the media write urb");
...
static int cimodule_probe(struct usb_interface *i_ptIntf, const struct usb_device_id *i_pId)
{
	...
	static struct usb_cimodule *s_ptDevData;
	...
	LOG_INFO("bIntfNum = %c matched module parameter, vendorID=0x%04x productID=0x%04x", bIntfNum,
	...
	if (i_ptIntf->cur_altsetting->desc.bInterfaceClass == COMMAND_INTF_CLASS
		...
		ptCmdInDesc = (usb_endpoint_is_bulk_in(&i_ptIntf->cur_altsetting->endpoint[0].desc)) ? &i_ptIntf->cur_altsetting->endpoint[0].desc : &i_ptIntf->cur_altsetting->endpoint[1].desc;
		ptCmdOutDesc = (usb_endpoint_is_bulk_out(&i_ptIntf->cur_altsetting->endpoint[1].desc)) ? &i_ptIntf->cur_altsetting->endpoint[1].desc : &i_ptIntf->cur_altsetting->endpoint[0].desc;

		LOG_CRIT("command interface: m1 =%d,m2=%d.\n", usb_endpoint_is_bulk_out(ptCmdOutDesc), usb_endpoint_is_bulk_in(ptCmdInDesc));
		LOG_CRIT("command interface: m3 =%d,m4=%d.\n", usb_endpoint_is_bulk_out(ptCmdInDesc), usb_endpoint_is_bulk_in(ptCmdOutDesc));
		...
		s_ptDevData->m_pbCmdInBuf = kmalloc(USB_CIMODULE_COMMAND_MAX_KMEM_SIZE, GFP_KERNEL);
		if (!s_ptDevData->m_pbCmdInBuf) {
			LOG_CRIT("usb dma memory alloc failed for command interface");
			nRetval = -ENOMEM;
			goto error;
		}
		...
		LOG_CRIT(" extra data size %d \n", nExtraSize);
		for (i = 0; i < nExtraSize ; i++) {
		   LOG_ERR("read data = 0x%x", *(pbExtraTemp++));
		}
		...
		s_ptDevData->m_pbConnectFlag = TRUE;
        alloc_write_command_buf = TRUE;
		...
		ptMdiInDesc = (usb_endpoint_is_bulk_in(&i_ptIntf->cur_altsetting->endpoint[0].desc)) ? &i_ptIntf->cur_altsetting->endpoint[0].desc : &i_ptIntf->cur_altsetting->endpoint[1].desc;
		ptMdiOutDesc = (usb_endpoint_is_bulk_out(&i_ptIntf->cur_altsetting->endpoint[1].desc)) ? &i_ptIntf->cur_altsetting->endpoint[1].desc : &i_ptIntf->cur_altsetting->endpoint[0].desc;
		LOG_CRIT("media interface: m1=%d,m2=%d.\n", usb_endpoint_is_bulk_out(ptMdiOutDesc), usb_endpoint_is_bulk_in(ptMdiInDesc));
		LOG_CRIT("media interface: m3=%d,m4=%d.\n", usb_endpoint_is_bulk_out(ptMdiInDesc), usb_endpoint_is_bulk_in(ptMdiOutDesc));
        ...
        /*
		s_ptDevData->m_pbMediaInBuf = kmalloc(512, GFP_KERNEL);
		if (!s_ptDevData->m_pbMediaInBuf) {
			LOG_CRIT("usb dma memory alloc failed for media interface");
			nRetval = -ENOMEM;
			goto error;
		}
		*/
		...
		s_ptDevData->m_pbConnectFlag = TRUE;
        alloc_write_buf = TRUE;
        alloc_read_buf = TRUE;
		LOG_CRIT("media interface register succeessfully,minor = %d\n", i_ptIntf->minor);
...
static void cimodule_disconnect(struct usb_interface *i_ptIntf)
{
	...
	LOG_INFO("cimodule_disconnect ptDevData %p\n", ptDevData);
	...
	ptDevData->m_pbConnectFlag = FALSE;
	...
	if (ptDevData->m_isCmdIntfReg
		...
		if (ptDevData->m_pbMediaOutBuf) {
	        usb_free_coherent(ptDevData->m_ptUsbDev, USB_CIMODULE_MEDIA_MAX_KMEM_SIZE, ptDevData->m_pbMediaOutBuf, ptDevData->m_pbMediaOutUrb->transfer_dma);
	        ptDevData->m_pbMediaOutBuf = NULL;
			LOG_CRIT("media interface free write buffer succeessfully.\n");
        }
		/*
		if (ptDevData->m_pbMediaInBuf){
            kfree(ptDevData->m_pbMediaInBuf);
			ptDevData->m_pbMediaInBuf = NULL;
			LOG_CRIT("media interface free read buffer succeessfully.\n");
		}
		*/
...
void usb_alloc_usbcam_buffer(void)
{
	m_pbMediaInBuf_init = kmalloc(USB_CIMODULE_MEDIA_MAX_KMEM_SIZE, GFP_KERNEL);
	if(m_pbMediaInBuf_init == NULL)
	{
		LOG_CRIT("usb_alloc_usbcam_buffer is fail.\n");
	}
	else
	{
		LOG_CRIT("usb_alloc_usbcam_buffer is ok.\n");
	}
}

EXPORT_SYMBOL_GPL(usb_alloc_usbcam_buffer);
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("USB CI+ 2.0 MODULE DRIVER");
