		    ...
		    struct nand_bbt_descr *td, int offs)
{
	...
	while (totlen) {
		...
		#if (MP_NAND_BBT_SIZE == 1)
		res = mtd_read(mtd, bbtpage, len, &retlen, buf);
		#else
			...
			struct nand_bbt_descr *td, int chip)
{
	...
	if (td->options & NAND_BBT_PERCHIP) {
		...
		for (i = 0; i < nanddev_ntargets(&this->base); i++) {
			if (chip == -1 || chip == i)
				#if (MP_NAND_BBT_SIZE == 1)
				res = read_bbt(this, buf, td->pages[i][0],
		...
		#if (MP_NAND_BBT_SIZE == 1)
		res = read_bbt(this, buf, td->pages[0][0],
			 ...
			 size_t len)
{
	...
	mtd->ooblayout->free(mtd, 0, &oobfree);
	if(oobfree.length < 4)
	{
			  ...
			  struct nand_bbt_descr *td, struct nand_bbt_descr *md)
{
	...
	if (td->options & NAND_BBT_VERSION) {
		...
		scan_read(this, buf, (loff_t)td->pages[0][0] << this->page_shift,
			      mtd->writesize, td);
	...
	if (md && (md->options & NAND_BBT_VERSION)) {
		#if (MP_NAND_BBT_SIZE == 1)
		#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)
		scan_read(this, buf, (loff_t)md->pages[0][0] << this->page_shift,
			      mtd->writesize, md);
		      ...
		      struct nand_bbt_descr *td)
{
	...
	int Secure_Reserved_Block_Cnt = (CONFIG_MSTAR_RESERVED_NAND_BYTE + (1<<this->bbt_erase_shift) - 1) >> this->bbt_erase_shift;
#endif
	...
	for (i = 0; i < chips; i++) {
		...
		#if defined(CONFIG_MSTAR_RESERVED_END_OF_NAND) && CONFIG_MSTAR_RESERVED_END_OF_NAND
		block = Secure_Reserved_Block_Cnt;
		for (; block < td->maxblocks; block++) {
		#else
			 ...
			 struct nand_bbt_descr *md, int chip)
{
...
#if defined(CONFIG_MSTAR_RESERVED_END_OF_NAND) && CONFIG_MSTAR_RESERVED_END_OF_NAND
	for (i = Secure_Reserved_Block_Cnt; i < td->maxblocks; i++) {
#else
	for (i = 0; i < td->maxblocks; i++) {
		...
		if(this->block_bad(&this->mtd, blockofs))
			continue;
		#endif
		#endif
		     ...
		     int chipsel)
{
	...
	while (chip < nrchips) {
		...
		for(k=0; k<bbtpage; k++){
			...
			#endif
			#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0)
			res = scan_write_bbt(this, lsbpage, mtd->writesize, pagebuf, &pagebuf[mtd->writesize]);
			#else
			...
			pr_info("Bad block table written to 0x%012llx, version 0x%02X\n",
				 (unsigned long long)lsbpage, td->version[chip]);
			td->pages[chip][k] = page;
		}
		chip ++;
		#else
