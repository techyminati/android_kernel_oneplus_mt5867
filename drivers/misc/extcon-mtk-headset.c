...
static int mtk_headset_extcon_probe(struct platform_device *pdev)
{
	...
	of_property_read_u32(dn, "extcon-gpios", &gpio_num);
...
int mtk_headset_extcon_suspend(struct platform_device *pdev, pm_message_t state)
{
	struct gpio_extcon_data *data = platform_get_drvdata(pdev);

	if (data == NULL) {
		pr_emerg("[%s][%d] data is null\n", __func__, __LINE__);
		goto exit;
	}
...
int mtk_headset_extcon_resume(struct platform_device *pdev)
{
	struct gpio_extcon_data *data = platform_get_drvdata(pdev);
...
void mtk_headset_extcon_shutdown(struct platform_device *pdev)
{
	struct gpio_extcon_data *data = platform_get_drvdata(pdev);
...
static struct platform_driver mtk_headset_extcon_driver = {
	...
	.suspend	= mtk_headset_extcon_suspend,
	.resume		= mtk_headset_extcon_resume,
	.shutdown	= mtk_headset_extcon_shutdown,
};
