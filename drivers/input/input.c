...
atomic_t ir_boost_duration = ATOMIC_INIT(4000);
		...
		if (is_event_supported(code, dev->keybit, KEY_MAX)) {
			...
			if(disposition & INPUT_PASS_TO_HANDLERS) {
				if (input_event_en==0x1){
					/* do not block the key release event */
					if(value != 0 && do_block(code,0,1)){
			...
			if (disposition & INPUT_PASS_TO_HANDLERS && value == 1) {
				mstar_cust_pwrkey_handler(code);
			}
#endif // defined(CONFIG_MSTAR_PM)
