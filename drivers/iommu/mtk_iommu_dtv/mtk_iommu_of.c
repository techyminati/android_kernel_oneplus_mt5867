		...
		uint64_t *addr1, uint64_t *size1)
{
	...
	struct lx_range_node lx_mem[MAX_LX_MEM];
	struct list_head *pos = NULL;
	struct lx_range_node *pos_node = NULL;
	...
	node = of_find_node_by_name(NULL, "cma_23");
	if (!node) {
		pr_err("%s,%d,cma_23 memory no found!\n",
			   __func__, __LINE__);
		free_lx_list(&lx_layout);
		return -1;
	}

		p64 = (__be64 *)of_get_property(node, "reg", &len);
		if (p64) {
...
struct list_head *mtk_iommu_get_buftags(void)
{
	...
	if (!mtk_iommu_buf_tag) {
		mtk_iommu_buf_tag = of_find_node_by_name(NULL, "buf_tag");
		if (!mtk_iommu_buf_tag) {
			pr_err("[IOMMU][%s] buf_tag not found!\n", __func__);
			return ERR_PTR(-EINVAL);
		}
		if (!mtk_iommu_buf_tag->child) {
			pr_err("[IOMMU][%s] buf_tag overlay failed!\n", __func__);
			goto out_node_put;
		}
	}
	...
	for (; np; np = np->sibling) {
		...
		ret = of_property_read_u32(np, "id", &buf_info->id);
		if (ret)
			goto out_node_put_child;

		ret = of_property_read_u32(np, "heaptype", &buf_info->heap_type);
		if (ret)
			goto out_node_put_child;

		ret = of_property_read_u32(np, "miu", &buf_info->miu);
		if (ret)
			goto out_node_put_child;

		ret = of_property_read_u64(np, "max_size", &buf_info->maxsize);
		if (ret)
			goto out_node_put_child;

		ret = of_property_read_u32(np, "normal_zone", &buf_info->zone);
		if (ret)
			goto out_node_put_child;

		ret = of_property_read_u32(np, "cache", &buf_info->cache);
		if (ret)
			goto out_node_put_child;
...
int mtk_iommu_get_buftag_info(struct buf_tag_info *info)
{
	...
	if (!head || list_empty(head)) {
		pr_err("[IOMMU][%s] buf_tag_list not found!\n", __func__);
		return -EINVAL;
	}
	...
	list_for_each_entry(buf, head, list) {
		...
		strncpy(info->name, buf->name, MAX_NAME_SIZE);
		info->name[MAX_NAME_SIZE - 1] = '\0';
		info->heap_type = buf->heap_type;
		info->miu = buf->miu;
		info->maxsize = buf->maxsize;
		info->zone = buf->zone;
		info->cache = buf->cache;
		found = true;
		break;
	}

	if (!found) {
		pr_err("[IOMMU][%s] buf_tag id: %d not found!\n", __func__, info->id);
		return -EINVAL;
	}
...
int mtk_iommu_get_mem_cust_info(struct mem_cust_policy_info *info)
{
	...
	if (!info) {
		pr_err("[IOMMU] mem_policy_info is null\n");
		return -EINVAL;
	}

	np = of_find_node_by_name(NULL, "mem_cust_policy");
	if (!np) {
		pr_err("[IOMMU] mem_cust_policy not found\n");
		return -EINVAL;
	}

	of_node_get(np);

	ret = of_property_read_u64(np, "cma_region_maxsize", &info->cma_region_maxsize);
	if (ret) {
		pr_err("[IOMMU] mem_cust_policy->cma_region_maxsize not found\n");
		goto out_node_put;
	}

	ret = of_property_read_u32(np, "mali_threshold", &info->mali_threshold);
	if (ret) {
		pr_err("[IOMMU] mem_cust_policy->mali_threshold not found\n");
		goto out_node_put;
	}

	ret = of_property_read_u32(np, "mali_reduce_migration", &info->mali_reduce_migration);
	if (ret) {
		pr_err("[IOMMU] mem_cust_policy->mali_reduce_migration not found\n");
		goto out_node_put;
	}

	tmp = of_property_count_elems_of_size(np, "need_2xiova", sizeof(uint32_t));
	if (tmp <= 0) {
		pr_err("[IOMMU] mem_cust_policy->need_2xiova not found\n");
		goto out_node_put;
	}

	info->nr_need_2xiova = tmp;

	info->need_2xiova = kcalloc(info->nr_need_2xiova, sizeof(uint32_t), GFP_KERNEL);
	if (!info->need_2xiova)
		goto out_node_put;

	for (i = 0; i < info->nr_need_2xiova; i++) {
		ret = of_property_read_u32_index(np, "need_2xiova", i, &tmp);
		if (ret) {
			pr_err("[IOMMU] mem_cust_policy->need_2xiova is not set\n");
			goto out_node_put;
		}

		info->need_2xiova[i] = tmp;
	}

	tmp = of_property_count_elems_of_size(np, "need_zeropage", sizeof(uint32_t));
	if (tmp <= 0) {
		pr_err("[IOMMU] mem_cust_policy->need_zeropage not found\n");
		goto out_node_put;
	}

	info->nr_need_zeropage = tmp;

	info->need_zeropage = kcalloc(info->nr_need_zeropage, sizeof(uint32_t), GFP_KERNEL);
	if (!info->need_zeropage)
		goto out_node_put;

	for (i = 0; i < info->nr_need_zeropage; i++) {
		ret = of_property_read_u32_index(np, "need_zeropage", i, &tmp);
		if (ret) {
			pr_err("[IOMMU] mem_cust_policy->need_zeropage is not set\n");
			goto out_node_put;
		}

		info->need_zeropage[i] = tmp;
	}
...
out_node_put:
	of_node_put(np);
	pr_err("[IOMMU] mem_cust_policy property not found\n");
	return -EINVAL;
}
...
int mtk_iommu_get_dummy(void)
{
	...
	np = of_find_node_by_name(NULL, "reserved_mpu_invalid");
	if (!np) {
		pr_info("[IOMMU] reserved_mpu_invalid not found\n");
		return -EINVAL;
	}
	of_node_get(np);

	ret = of_property_read_u32_index(np, "reg", DUMMY_ADDR_REG, &addr);
	if (ret) {
		pr_err("[IOMMU] reserved_mpu_invalid addr is not set\n");
		goto out_node_put;
	}

	ret = of_property_read_u32_index(np, "reg", DUMMY_SIZE_REG, &size);
	if (ret) {
		pr_err("[IOMMU] reserved_mpu_invalid size is not set\n");
		goto out_node_put;
	}

	if (size != DUMMY_SIZE) {
		pr_err("[IOMMU] dummy page size :%x is invalid\n", size);
		goto out_node_put;
	}

	/* set value in dummy reg */
	base = ioremap_wc(CPU_DUMMY, SZ_4K);
	if (!base) {
		pr_err("[IOMMU] Unable to ioremap dummy reg\n");
		goto out_node_put;
	}

	if ((READ_REG(0) != 0)) {
		pr_err("[IOMMU] dummy reg :%x in used\n", READ_REG(CPU_DUMMY));
		goto out_node_put;
	}

	WRITE_REG_16(addr / SZ_1M, CPU_DUMMY_0);
	WRITE_REG_16(size / SZ_1M, CPU_DUMMY_1);
