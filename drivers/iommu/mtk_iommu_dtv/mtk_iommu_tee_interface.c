...
enum {
	NORMAL_CMD_SESSION = 0,
	MAP_IOVA_CMD_SESSION,
	UNMAP_IOVA_CMD_SESSION,
	MAX_SESSIONS_NR,
};

static struct mtk_iommu_optee_contex mtk_iommu_ctx[MAX_SESSIONS_NR];
		...
		struct TEEC_SharedMemory_2 *shm)
{
	...
	if (!len)
		len = 8;
	_shm = tee_shm_alloc(ctx, len, TEE_SHM_MAPPED | TEE_SHM_DMA_BUF);
	if (IS_ERR(_shm)) {
...
static int mtk_iommu_init_context(void)
{
	...
	for (i = 0; i < MAX_SESSIONS_NR; i++) {
		if (!IS_ERR_OR_NULL(mtk_iommu_ctx[i].ctx))
			return 0;

		mtk_iommu_ctx[i].ctx = tee_client_open_context(NULL, optee_ctx_match, NULL,
								NULL);
		if (IS_ERR_OR_NULL(mtk_iommu_ctx[i].ctx)) {
			ret = PTR_ERR(mtk_iommu_ctx[i].ctx);
			pr_err("%s: tee_client_open_context id: %d, failed with %d\n",
					__func__, i, ret);
			return -ENODEV;
		}
	}
...
#define UUID_PART_1 0x59346609
#define UUID_PART_2 0xbc83
#define UUID_PART_3 0x4d43
#define UUID_PART_4 0xb6
#define UUID_PART_5 0x26
#define UUID_PART_6 0x13
#define UUID_PART_7 0x08
#define UUID_PART_8 0x21
#define UUID_PART_9 0x9a
#define UUID_PART_10 0x16
#define UUID_PART_11 0xad
static int mtk_iommu_open_id_session(unsigned int i, u32 *session_id)
{
	int res;
	struct tee_ioctl_open_session_arg arg = { };
	uuid_t uuid = UUID_INIT(
			UUID_PART_1,
			UUID_PART_2,
			UUID_PART_3,
			UUID_PART_4,
	...
	res = tee_client_open_session(mtk_iommu_ctx[i].ctx, &arg, NULL);
	if (res) {
	...
	mtk_iommu_ctx[i].session_initialized = true;
		...
		struct TEEC_Operation *operation, uint32_t *return_origin)
{
	...
	if (commandID == MAP_IOVA)
		id = MAP_IOVA_CMD_SESSION;
	else if (commandID == UNMAP_IOVA)
		id = UNMAP_IOVA_CMD_SESSION;
	else
		id = NORMAL_CMD_SESSION;

	if (mtk_iommu_ctx[id].session_id == 0) {
		res = mtk_iommu_open_id_session(id, &mtk_iommu_ctx[id].session_id);
		if (res)
			return res;
	}

	arg.num_params = TEE_PARAM_COUNT;
	arg.session = mtk_iommu_ctx[id].session_id;
	arg.func = commandID;

	res = teec_pre_process_operation(mtk_iommu_ctx[id].ctx,
	...
	res = tee_client_invoke_func(mtk_iommu_ctx[id].ctx, &arg, params);
	if (res) {
		eorig = 2;
		goto out;
	}
...
static void mtk_iommu_destroy_session(void)
{
	...
	for (i = 0; i < MAX_SESSIONS_NR; i++) {
		if (!mtk_iommu_ctx[i].session_initialized) {
			pr_err("%s: tee_client_destroy_session id : %d, not initialize.\n",
			__func__, i);
			continue;
		}
		tee_client_close_context(mtk_iommu_ctx[i].ctx);
		mtk_iommu_ctx[i].session_initialized = false;
	}
}
...
int mtk_iommu_tee_open_session(void)
{
	...
	if (cnt == MAX_SESSIONS_NR) {
		pr_info("mma ta number of sessions: %d already init\n", cnt);
		return 0;
	}
	...
	for (i = 0; i < MAX_SESSIONS_NR; i++) {
		ret = mtk_iommu_open_id_session(i, &mtk_iommu_ctx[i].session_id);
		if (ret)
			return ret;
	}
