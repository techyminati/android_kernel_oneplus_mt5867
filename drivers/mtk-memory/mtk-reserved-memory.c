...
#if IS_ENABLED(CONFIG_ION)
/* this code is copy from ion driver,
 * ex: ion_alloc_fd()
 */
...
#if IS_ENABLED(CONFIG_DMABUF_HEAPS_CMA) || IS_ENABLED(CONFIG_DMABUF_HEAPS)
/* Since we can use dma_heap_bufferfd_alloc() directly,
 * so we do not need to have mtk_ion_alloc.
 * Here, we keep only dmaheap_test().
 */
int dmaheap_test(struct device *dev, unsigned long alloc_offset,
		unsigned long size, unsigned int fd_flags, unsigned int heap_flags)
{
	struct dma_heap *heap;
	int fd = -1;

	/* step_1: get dma_heap with device_name,
	 * this dma_heap is created by mtkcma driver
	 */
	heap = dma_heap_find(dev_name(dev));
	if (!heap) {
		pr_err("[MTKCMA][%s]: could not find mtkcma dmaheap for device %s\n",
				__func__, dev_name(dev));
		return -ENODEV;
	}

	/* step_2: alloc memory from cma by mtkcma dmaheap.
	 * fd_flags must be DMA_HEAP_VALID_FD_FLAGS.
	 * heap_flags must be DMA_HEAP_VALID_HEAP_FLAGS.
	 * one important thing is dma_heap_bufferfd_alloc()
	 * will not support us to specify a fixed range of cma_allocation.
	 */
	fd = dma_heap_bufferfd_alloc(heap, size, fd_flags, heap_flags);

	pr_info("[ION] get fd %d\n", fd);
	return fd;
}
EXPORT_SYMBOL(dmaheap_test);
#endif
