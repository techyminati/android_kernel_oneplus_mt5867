		...
		dma_addr_t *alloc_offset, gfp_t flag, unsigned long attrs)
{
	...
	if (IS_ERR_OR_NULL(va_ptr)) {
		pr_emerg("Function = %s, fail, va_ptr is 0x%pX, err\n",
...
static int mmap_userdev_open(struct inode *inode, struct file *file)
{
	...
	for (i = 0; i < TEST_CNT; i++) {
		...
		pr_info("[%d] vaddr is 0x%pX\n", i, vaddr);

		memset(vaddr, 0, of_mmap_info.buffer_size);
		pr_info("value is 0x%X\n", *(unsigned int *)vaddr);
...
#if IS_ENABLED(CONFIG_DMABUF_HEAPS_CMA) || IS_ENABLED(CONFIG_DMABUF_HEAPS)
	pr_emerg("user mode cma_alloc (dmaheap), start\n");
	fd = dmaheap_test(mtk_test_mmap_device, ion_alloc_offset,
		PAGE_ALIGN(TEST_1MB), DMA_HEAP_VALID_FD_FLAGS, DMA_HEAP_VALID_HEAP_FLAGS);

	if (fd < 0) {
		pr_emerg("test dmaheap ops failed, return\n");
		return -EBADF;
	}
	buffer_pfn = dmaheap_get_mtkcma_buffer_info(fd);
	pr_emerg("ion get: buffer_phy is 0x%lX\n", buffer_pfn << PAGE_SHIFT);
	close_fd(fd);
	pr_emerg("user mode cma_alloc (dmaheap), end\n\n\n");
#endif

	// test_3: dma_ops
	pr_emerg("kernel mode cma_alloc (dma_alloc_attrs), start\n");
	dma_alloc_offset = DMA_ALLOC_OFFSET;
