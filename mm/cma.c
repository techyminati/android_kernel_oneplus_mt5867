                       ...
                       unsigned int align, long *retlen)
{
	...
	/* FIXME: GFP_KERNEL */
        ret = alloc_contig_range(pfn, pfn + len, MIGRATE_CMA, GFP_KERNEL, &dummy);
...
#endif
		/* FIXME: GFP_KERNEL */
		ret = alloc_contig_range(pfn, pfn + count, MIGRATE_CMA, GFP_KERNEL, &dummy);
#ifdef CONFIG_MP_CMA_PATCH_COUNT_TIMECOST
