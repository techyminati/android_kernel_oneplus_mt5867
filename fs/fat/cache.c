...
int fat_fix_inode_cluster(struct inode *inode, int *fclus, int max_clus)
{
	...
	*fclus = 0;
	...
	if (0 == dclus) {
		...
		fat_msg_ratelimit(sb, KERN_ERR, "dclus %08x >=  sbi->max_cluster %08lx\n", dclus, sbi->max_cluster);

		//fat_fs_error_ratelimit maybe set sb->s_flags |= MS_RDONLY
		//keep the same phenomenon with official source code
		//fat_fs_error_ratelimit(sb, "dclus %08x >=  sbi->max_cluster %08lx\n", dclus, sbi->max_cluster);
		return 0;
	} else if (dclus < FAT_START_ENT) {
		fat_msg_ratelimit(sb, KERN_ERR, "invalid access to FAT (entry 0x%08x)", dclus);
		return 0;
	}
